...
- Conventional single-cell (not spatial spots) cell-type proportion comparison → standalone `miloR` / `scCODA` (compositional DA, see omicverse-pipeline DA section)
...
| `'RCTD'` | RCTD (R/spacexr wrapper) | doublet mode is its strength | none (CPU) | ❌ `pip install spacexr` or run in R env |
