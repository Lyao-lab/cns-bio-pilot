...
> （其中 **215 篇**提取到完整 Fig1-FigN 原文结构）。原始调研档案：`D:\workspace\lit_survey\01-15_*.md`（每篇含期刊/年份/PMID/平台/figure 标题，未获取处如实标注，无编造）。
...
- 主图数量中位数（215 篇）：**6 张**；最紧凑 3-4（Nature 正刊方法/机制），最多 8（Nat Commun 机制做满）。
