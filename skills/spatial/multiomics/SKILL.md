...
> **Visium HD bin2cell**: full workflow (load → cellpose nuclei → expand → aggregate → sync geometries → quality check) is in `examples/visium_hd_bin2cell.py`. **Verified API** (version in `compat.yaml`): `ov.io.read_visium_hd` / `ov.space.visium_10x_hd_cellpose_expand` / `ov.space.bin2cell` / `ov.space.sync_visium_hd_seg_geometries`.
